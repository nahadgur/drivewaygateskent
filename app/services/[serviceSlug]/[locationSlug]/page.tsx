...
const serviceLocationContent: Record<string, {
  ...
  'electric-sliding': {
    ...
    whyPoints: (city) => [
      `Sliding gate specialists in our ${city} network have hands-on experience with the ground conditions, gradients, and property types found across this part of Kent.`,
      ...
      `FAAC, BFT, CAME, and Nice motor systems available, with established Kent parts networks supporting long-term maintenance.`,
    ],
  },
  'electric-swing': {
    intro: (city) => [
    ...
    whyPoints: (city) => [
      `Swing gate installers in our ${city} network understand the property types, plot sizes, and gradient conditions typical across this part of Kent.`,
  ...
  'wooden-gates': {
    intro: (city) => [
      `Hardwood gates are the instinctive specification for a large proportion of ${city} properties. The High Weald AONB, the Green Belt villages, the conservation areas in and around the county's market towns, and the substantial stock of period and rural residential properties across the county all create contexts where a natural timber gate is not just aesthetically preferable but often the only material that feels genuinely right. Where a planning authority or a conservation officer is in the picture, timber is also frequently the specification that raises least resistance.`,
      `The fabrication approach for hardwood gates in ${city} is almost always bespoke rather than catalogue. A skilled timber gate maker can produce a gate that reflects the proportions of the entrance, references the architectural character of the house, or achieves a specific design detail that matters to the homeowner, with a relatively modest premium over a standard size. Iroko is the default timber for most projects: it is naturally durable, stable across Kent's seasonal moisture cycle, and available in consistent quality. European oak is specified where the grain character is a deliberate part of the brief. Accoya carries a 50-year manufacturer durability certificate and is the right choice for exposed positions and for homeowners who want timber aesthetics without the maintenance schedule.`,
    ],
    steps: (city) => [
    ...
    whyPoints: (city) => [
      `Hardwood gate specialists in our ${city} network understand Kent's AONB and conservation area planning context and can advise on the right specification before you commit.`,
  ...
  'metal-gates': {
    intro: (city) => [
      `Metal gate installations in ${city} span a wider range than in most markets. At one end, the high-specification residential streets of North and East Kent produce some of the most ambitious wrought iron entrance treatments in the country: hand-forged gates on brick piers with gilded finials, underground motors, HD video intercom, and proximity access that would not look out of place on a managed estate. At the other end, fabricated steel or aluminium sliding gates in anthracite powder coat are a clean, practical, and lower-maintenance answer for modern builds and commuter belt properties where the brief is functional rather than decorative.`,
  ...
  'automated-systems': {
    ...
    whyPoints: (city) => [
      ...
      `FAAC, BFT, CAME, Nice, and Beninca motor systems available, with established parts networks across Kent for long-term support.`,
