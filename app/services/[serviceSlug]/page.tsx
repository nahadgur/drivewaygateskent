...
const serviceContent: Record<string, { intro: string[]; benefits: { title: string; desc: string }[]; candidateIntro: string; candidates: string[]; process: { title: string; desc: string }[] }> = {
  'electric-sliding': {
    intro: [
      "Kent driveways present a specific set of challenges that make sliding gates the right answer more often than in most counties. The county has a large number of sloped driveways, particularly across the North Downs between Reigate and Guildford, the High Weald between Dorking and Haslemere, and the hillside streets of Sevenoaks, Westerham, and the Downs villages. A swing gate on a significant gradient requires complex hinge engineering and careful motor specification; a sliding gate sidesteps the problem entirely by moving horizontally along the boundary rather than sweeping through a vertical arc.",
      ...
      "Run-back space is the main planning constraint. The gate needs a clear zone alongside the opening at least as wide as the gate leaf itself, with an additional 500mm or so for the motor housing and mechanical end stops. On many Kent properties this space is readily available as a boundary wall or close-boarded fence runs alongside the opening. Where it is not, a biparting layout splits the gate into two leaves that slide in opposite directions, halving the run-back needed on each side.",
    ],
    benefits: [
      { title: 'Works on Sloped Kent Driveways', desc: 'Where swing gates are defeated by gradient, a sliding gate simply tracks along the boundary. Cantilever systems eliminate the ground constraint entirely and are well-suited to the varied terrain found across the North Downs and High Weald.' },
      ...
      { title: 'Compact Footprint, Clean Appearance', desc: 'When open, the gate leaf sits flat against the boundary with nothing projecting into the driveway. A significant practical advantage on short approaches common in Kent commuter towns.' },
  ...
  'electric-swing': {
    intro: [
      ...
      "The suitability check for a swing gate comes down to clearance. The gate leaf needs to complete its full 90-degree arc without hitting parked vehicles, steps, raised borders, or structures within the swing path. Kent driveways with gradients, particularly those sloping toward the property, add a further consideration: the bottom of the gate leaf will describe a path that comes closer to the slope as it opens, which requires the motor specification and hinge geometry to account for the gradient. Your installer will walk the arc on site before specifying anything.",
    ],
    benefits: [
      { title: 'Stronger Architectural Presence', desc: 'Paired gates opening symmetrically from a central point make a statement that a single sliding leaf does not. The format suits Kent period properties, detached family homes, and any entrance where the gate is meant to be seen.' },
      ...
      { title: 'Underground Motor Option', desc: 'The concealed motor approach is more widely specified in Kent than most counties. On premium properties in Sevenoaks, Tunbridge Wells, and Westerham, underground motors are the standard choice rather than the exception.' },
    ...
    candidateIntro: "Swing gates suit most Kent residential properties where the driveway allows it:",
    candidates: [
  ...
  'wooden-gates': {
    intro: [
      ...
      "Three timber species account for almost all Kent residential gate installations. Iroko is the practical choice for most situations: it is naturally durable to Use Class 3 without reliance on applied treatment, it holds its shape well through the seasonal weather cycles of the South East, and it accepts oil treatment uniformly across its surface. European oak brings greater character to the grain and is the preference when the timber itself is a design feature rather than a structure to be finished. Accoya is the specification when low maintenance is a firm requirement: the acetylation process so fundamentally changes how the timber responds to moisture that the manufacturer certifies 50 years above-ground durability, and in exposed rural positions in the High Weald this genuinely matters.",
      "Bespoke fabrication is the norm for timber gates in Kent, not a premium option. A skilled maker can work to a precise brief for a fraction of the additional cost that bespoke metalwork would require, and the output can reference the proportions of your gateposts, the profile of your fencing, or a detail taken from the house itself. Most Kent joinery firms specialising in gates will produce drawings for approval before cutting a single piece of timber.",
    ],
    benefits: [
      { title: 'The Natural Choice for Kent Heritage Properties', desc: 'Conservation areas, listed buildings, AONB settings, and properties where the local planning context favours traditional materials all benefit from timber gates. In these contexts, hardwood is not just aesthetically superior; it is often the specification that planning advisers expect.' },
      { title: 'Genuinely Bespoke at an Accessible Price', desc: 'The joinery skills required for hardwood gate fabrication are more widely available than specialist metalworking. A one-off design that exactly suits your property adds modest cost over a standard size and is achievable through most Kent timber gate makers.' },
      { title: 'Privacy and Acoustic Benefit', desc: 'A close-boarded hardwood gate gives complete visual privacy and meaningful reduction of road noise, which is relevant on Kent properties fronting A-roads or in areas with consistent traffic flow.' },
  ...
  'metal-gates': {
    intro: [
      "Kent is one of the strongest markets for metal driveway gates in England, and the county illustrates the full range of what the material category covers. At the top end of the market, hand-forged wrought iron gates on brick piers with gilded finials and underground motors are a recurring specification in the Sevenoaks, Tunbridge Wells, and Westerham area. At the other end, a precision-cut aluminium sliding gate in anthracite powder coat is the functional, contemporary solution for a new-build in Dartford or Maidstone. Between these points sits the majority of Kent metal gate installations: mild steel fabricated to a bespoke design, hot-dip galvanised, and finished in whatever colour the property requires.",
  ...
  'automated-systems': {
    ...
    benefits: [
      ...
      { title: 'Underground Motors Available on Most Kent Retrofits', desc: 'The preference for concealed motors in the Kent market is well-established, and most retrofit sites in the county can accommodate underground installation with relatively minor additional work.' },
    ...
    candidates: [
      ...
      "Vehicle security is a concern and a closed automated gate is a meaningful deterrent against the relay theft methods common in Kent",
  ...
  'gate-repair': {
    intro: [
      "Gate faults in Kent follow predictable patterns. Motor and gearbox wear from inadequate lubrication or under-specification for the gate weight. Control board failure from moisture ingress, usually through a poorly sealed housing or a cable entry point that was not adequately weatherproofed. Photocell misalignment after vehicle impact or frost movement, causing the gate to stop mid-travel when there is no obstruction. Hinge wear on gates that were never correctly adjusted at installation, creating a sagging leaf that the motor has to fight to move. Track contamination on sliding gates, where leaf litter, grit, and debris build up in the channel until the rollers bind.",
      "The majority of these faults are resolved in a single visit by an engineer who carries diagnostic equipment and the common spare parts for the brands most widely installed in Kent: FAAC, BFT, CAME, Nice, and Beninca. The brands matter because parts availability for any given motor is determined by the manufacturer support infrastructure, and the established brands all maintain active spare parts supply for their product ranges for a minimum of ten years after production ends. Unbranded or obscure motors installed by less experienced firms create a recurring problem: the motor fails and the parts to fix it are either not available or need to be sourced from overseas at significant delay and cost.",
    ...
    benefits: [
      ...
      { title: 'Stock Parts for Kent Market Brands', desc: 'Engineers carry parts for FAAC, BFT, CAME, Nice, and Beninca. The majority of Kent callouts are resolved on the first visit because the right component is on the van.' },
